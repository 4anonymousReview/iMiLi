...
public class Rule {
	public int situation(String code, Generation.CH C, String res, String res2, int line)
	{
		...
		if(rule_3(code,C,res,res2,line)>0)return rule_3(code,C,res,res2,line);
		...
		if(rule_2(code,C,res,res2,line)>0)return rule_2(code,C,res,res2,line);
		if(rule_3(code,C,res,res2,line)>0)return rule_3(code,C,res,res2,line);
...
//		System.out.println("RULE4");
		if(true)
		{
			...
			int i1=C.p;
			int i2=C.p+1;
			if(i1<0||i1>=code.length()) return 0;
...
//					System.out.println("NOPE:"+search_String(code,list.get(i)));
	...
	{
		if(C.c2.contains("import")&&res.contains("cannot be resolved"))
		return 3;
		if(l2s(C.code,line).contains("import")&&res.contains("cannot be resolved"))
		return 3;
		else return 0;
	}
	public String l2s(String code, int line)
	{
		int i=0;
		int p1=0,p2=0;
	...
	public int rule_2(String code,Generation.CH C, String res, String res2, int line)///**/的匹配
	{
		int i=0;
		String s;
		if(res.contains("Unexpected end of comment"))
			return 2;

		ArrayList<Integer> list=search_line(res,res2);//放入所有的报错行数
		for(i=0;i<list.size();i++)
		{
			s=l2s(C.code,list.get(i));
	...
	public int rule_1(String code,Generation.CH C, String res, String res2, int line)//{}的匹配
	{
		...
		for (int i=0;i<list.size();i++)
		{
			if(res.contains("{")||res.contains("}")) return 1;
